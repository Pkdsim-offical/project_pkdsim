...
using project_pkdsim.Assets.USys.USystemsSys.ITEMS.ItemsScripts.Gifts.Necklaces.Butterfly_necklace;
using UnityEngine;

namespace project_pkdsim.Assets.USys.USystemsSys.ITEMS.ItemsScripts.Gifts.Necklaces.I.Properties.LoadJson.Butterfly_necklace
{
    public class Butterfly_necklaceJsonLoader : MonoBehaviour
    {
        private Butterfly_NecklaceItem necklaceData;
        public void LoadJson()
        {
            string filePath = Path.Combine(Application.dataPath, "Resources", "Butterfly_necklace.json");
            if (File.Exists(filePath))
            {
                string jsonContent = File.ReadAllText(filePath);
                necklaceData = JsonConvert.DeserializeObject<Butterfly_NecklaceItem>(jsonContent);
                Debug.Log("Loaded Butterfly_necklace data successfully.");
            }
            else
            {
                Debug.LogError("Butterfly_necklace.json file not found.");
            }
        }
        ...
        public void ButterflyNecklace_LoveSys_Stager()
        {
            LoadJson();
            ...
            int specialExp = LoveSys_Special_States("Unknown");
            Debug.Log($"EXP change for Unknown: {specialExp}");
        }
    }
}