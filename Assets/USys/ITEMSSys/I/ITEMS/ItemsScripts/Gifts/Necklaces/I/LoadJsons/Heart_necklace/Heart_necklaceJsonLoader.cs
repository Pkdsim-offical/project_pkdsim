using project_pkdsim.Assets.USys.USystemsSys.ITEMS.ItemsScripts.Gifts.Necklaces.Heart_necklace;
using project_pkdsim.Assets.USys.USystemsSys.ITEMS.ItemsScripts.Gifts.Necklaces.I.Base_Properties;
using System.IO;
using Newtonsoft.Json;
using UnityEngine;

namespace project_pkdsim.Assets.USys.USystemsSys.ITEMS.ItemsScripts.Gifts.Necklaces.I.Properties.LoadJson.Heart_necklace
{
    public class Heart_necklaceJsonLoader : MonoBehaviour
    {
        private Heart_NecklaceItem necklaceData;
        public void LoadJson()
        {
            string filePath = Path.Combine(Application.dataPath, "Resources", "Heart_necklace.json");
            if (File.Exists(filePath))
            {
                string jsonContent = File.ReadAllText(filePath);
                necklaceData = JsonConvert.DeserializeObject<Heart_NecklaceItem>(jsonContent);
                Debug.Log("Loaded Heart_necklace data successfully.");
            }
            else
            {
                Debug.LogError("Heart_necklace.json file not found.");
            }
        }
        ...
        public void HeartNecklace_LoveSys_Stager()
        {
            LoadJson();
            ...
            int specialExp = LoveSys_Special_States("Sylveon");
            Debug.Log($"EXP change for Sylveon: {specialExp}");
        }
    }
}