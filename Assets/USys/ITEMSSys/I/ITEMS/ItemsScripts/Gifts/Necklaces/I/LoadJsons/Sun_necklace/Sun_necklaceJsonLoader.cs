using project_pkdsim.Assets.USys.USystemsSys.ITEMS.ItemsScripts.Gifts.Necklaces.Sun_necklace;
using project_pkdsim.Assets.USys.USystemsSys.ITEMS.ItemsScripts.Gifts.Necklaces.I.Base_Properties;
using System.IO;
using Newtonsoft.Json;
using UnityEngine;

namespace project_pkdsim.Assets.USys.USystemsSys.ITEMS.ItemsScripts.Gifts.Necklaces.I.Properties.LoadJson.Sun_necklace
{
    public class Sun_necklaceJsonLoader : MonoBehaviour
    {
        private Sun_NecklaceItem necklaceData;
        private void LoadJson()
        {
            string filePath = Path.Combine(Application.dataPath, "Resources", "Sun_necklace.json");
            if (File.Exists(filePath))
            {
                string jsonContent = File.ReadAllText(filePath);
                necklaceData = JsonConvert.DeserializeObject<Sun_NecklaceItem>(jsonContent);
                Debug.Log("Loaded Sun_necklace data successfully.");
            }
            else
            {
                Debug.LogError("Sun_necklace.json file not found.");
            }
        }
        ...
        public void SunNecklace_LoveSys_Stager()
        {
            LoadJson();
