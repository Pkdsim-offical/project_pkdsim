using project_pkdsim.Assets.USys.ITEMSSys.ITEMS.ItemsScripts.Gifts.Necklaces.Bolt_necklace;
using project_pkdsim.Assets.USys.ITEMSSys.ITEMS.ItemsScripts.Gifts.Necklaces.I.Base_Properties;
using System.IO;
using Newtonsoft.Json;
using UnityEngine;
namespace project_pkdsim.Assets.USys.ITEMSSys.ITEMS.ItemsScripts.Gifts.Necklaces.I.LoadJson.Bolt_necklace
{
	public class Bolt_necklaceJsonLoader : MonoBehaviour
	{
		private Bolt_NecklaceItem necklaceData;

		public void LoadJson()
		{
			string filePath = Path.Combine(Application.dataPath, "Resources", "Bolt_necklace.json");
			if (File.Exists(filePath))
			{
				string jsonContent = File.ReadAllText(filePath);
				necklaceData = JsonConvert.DeserializeObject<Bolt_NecklaceItem>(jsonContent);
				Debug.Log("Loaded Bolt_necklace data successfully.");
			}
			else
			{
				Debug.LogError("Bolt_necklace.json file not found.");
			}
		}
		...
		public void BoltLoveSys_Stager()
		{
			...
			int specialExp = LoveSys_Special_States("Jolteon");
			Debug.Log($"EXP change for Jolteon: {specialExp}");
		}
	}
}