using project_pkdsim.Assets.USys.ITEMSSys.ITEMS.ItemsScripts.Gifts.Necklaces.PunchingBag_necklace;
using System.IO;
using Newtonsoft.Json;
using UnityEngine;
using project_pkdsim.Assets.USys.ITEMSSys.ITEMS.ItemsScripts.Gifts.Necklaces.I.Base_Properties;

namespace project_pkdsim.Assets.USys.ITEMSSys.ITEMS.ItemsScripts.Gifts.Necklaces.I.LoadJson.PunchingBag_necklace
{
	public class PunchingBag_NecklaceJsonLoader : MonoBehaviour 
	{
		...
		{
			string filePath = Path.Combine(Application.dataPath, "Resources", "PunchingBag_necklace.json");
			if (File.Exists(filePath))
			{
				string jsonContent = File.ReadAllText(filePath);
				necklaceData = JsonConvert.DeserializeObject<PunchingBag_NecklaceItem>(jsonContent);
				Debug.Log("Loaded PunchingBag_necklace data successfully.");
			}
			else
			{
				Debug.LogError("PunchingBag_necklace.json file not found.");
			}
		}
		...
		public void PunchingBagNecklace_LoveSys_Stager()
		{
			LoadJson();
