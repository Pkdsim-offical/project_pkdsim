using project_pkdsim.Assets.USys.USystemsSys.ITEMS.ItemsScripts.Gifts.Necklaces.Crust_necklace;
using project_pkdsim.Assets.USys.USystemsSys.ITEMS.ItemsScripts.Gifts.Necklaces.I.Base_Properties;
using System.IO;
using Newtonsoft.Json;
using UnityEngine;

namespace project_pkdsim.Assets.USys.USystemsSys.ITEMS.ItemsScripts.Gifts.Necklaces.I.LoadJson.Crust_necklace
{
    public class Crust_necklaceJsonLoader : MonoBehaviour
    {
         private Crust_NecklaceItem necklaceData;

        public void LoadJson()
        {
            string filePath = Path.Combine(Application.dataPath, "Resources", "Crust_necklace.json");
            if (File.Exists(filePath))
            {
                string jsonContent = File.ReadAllText(filePath);
                necklaceData = JsonConvert.DeserializeObject<Crust_NecklaceItem>(jsonContent);
                Debug.Log("Loaded Crust_necklace data successfully.");
            }
            else
            {
                Debug.LogError("Crust_necklace.json file not found.");
            }
        }
        ...
        public void CrustNecklace_LoveSys_Stager()
        {
            LoadJson();
