...
namespace project_pkdsim.USys.UItemsSystemSys.Modules.ITEMS.ItemsScripts.Gifts.Necklaces.Modules.LoadJson.Fighting.Punching_Bag_necklace
{
	public class Punching_Bag_necklaceJsonLoader : MonoBehaviour 
	{
		private Punching_Bag_NecklaceItem necklaceData;

		public void LoadJson()
		{
			string filePath = Path.Combine(Application.dataPath, "Resources", "Punching_Bag_necklace.json");
			if (File.Exists(filePath))
			{
				string jsonContent = File.ReadAllText(filePath);
				necklaceData = JsonConvert.DeserializeObject<Punching_Bag_NecklaceItem>(jsonContent);
				Debug.Log("Loaded Punching_Bag_necklace data successfully.");
			}
			else
			{
				Debug.LogError("Punching_Bag_necklace.json file not found.");
			}
		}
		...
		public void Punching_BagNecklace_LoveSys_Stager()
		{
			LoadJson();
			...
			int specialExp = LoveSys_Special_States("Lucario");
