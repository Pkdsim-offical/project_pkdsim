...
namespace project_pkdsim.USys.UItemsSystemSys.Modules.ITEMS.ItemsScripts.Gifts.Necklaces.Modules.LoadJson.Ghost.Soul_necklace
{
	public class Soul_necklaceJsonLoader : MonoBehaviour
	{
		private Soul_NecklaceItem necklaceData;
		private void LoadJson()
		{
			string filePath = Path.Combine(Application.dataPath, "Resources", "Soul_necklace.json");
			if (File.Exists(filePath))
			{
				string jsonContent = File.ReadAllText(filePath);
				necklaceData = JsonConvert.DeserializeObject<Soul_NecklaceItem>(jsonContent);
				Debug.Log("Loaded Soul_necklace data successfully.");
			}
			else
			{
				Debug.LogError("Soul_necklace.json file not found.");
			}
		}
		...
		public void SoulNecklace_LoveSys_Stager()
		{
			LoadJson();
