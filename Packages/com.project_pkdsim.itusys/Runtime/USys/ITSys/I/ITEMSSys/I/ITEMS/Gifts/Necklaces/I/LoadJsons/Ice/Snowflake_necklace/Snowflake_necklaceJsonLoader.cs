...
namespace project_pkdsim.USysUItemsSystemSys.Modules.ITEMS.ItemsScripts.Gifts.Necklaces.Modules.LoadJsons.Ice.Snowflake_necklace
{
	public class Snowflake_necklaceJsonLoader : MonoBehaviour
	{
		private Snowflake_NecklaceItem necklaceData;
		protected void LoadJson()
		{
			string filePath = Path.Combine(Application.dataPath, "Resources", "Snowflake_necklace.json");
			if (File.Exists(filePath))
			{
				string jsonContent = File.ReadAllText(filePath);
				necklaceData = JsonConvert.DeserializeObject<Snowflake_NecklaceItem>(jsonContent);
				Debug.Log("Loaded Snowflake_necklace data successfully.");
			}
			else
			{
				Debug.LogError("Snowflake_necklace.json file not found.");
			}
		}
		...
		public void SnowflakeNecklace_LoveSys_Stager()
		{
			LoadJson();
			...
			int specialExp = LoveSys_Special_States("Cinderace");
			Debug.Log($"Love_EXP change for Cinderace: {specialExp}");
		}
	}
}